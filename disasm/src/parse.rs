use crate::*;
impl From<u32> for BranchTarget {
    fn from(value: u32) -> Self {
        Self { addr: ((value)) as i32 }
    }
}
...
pub fn parse_arm(ins: u32) -> Ins {
    if (ins & 0xde00000) == 0xa00000 {
        ...
        parse_arm_and_0(ins as u32)
    } else if (ins & 0xf000000) == 0xa000000 {
        parse_arm_b_0(ins as u32)
    } else {
        Ins::Illegal
    }
}
pub fn parse_thumb(ins: u16, next: Option<u16>) -> Ins {
    if (ins & 0xffc0) == 0x4140 {
        ...
        parse_thumb_and_0(ins as u32)
    } else if (ins & 0xf000) == 0xd000 {
        parse_thumb_b_0(ins as u32)
    } else if (ins & 0xf800) == 0xe000 {
        parse_thumb_b_1(ins as u32)
    } else {
        Ins::Illegal
    }
}
...
fn parse_arm_and_0(value: u32) -> Ins {
    let s = (((value) >> 20) & 0x1) != 0;
    let cond = Cond::from(((value) >> 28) & 0xf);
    let rd = Reg::from(((value) >> 12) & 0xf);
    let rn = Reg::from(((value) >> 16) & 0xf);
...
fn parse_thumb_and_0(value: u32) -> Ins {
    let s = (1) != 0;
    let cond = Cond::default();
    let rd = Reg::from((value) & 0x7);
    let rn = Reg::from((value) & 0x7);
...
fn parse_arm_b_0(value: u32) -> Ins {
    let cond = Cond::from(((value) >> 28) & 0xf);
    let target = BranchTarget {
        addr: (((((((value) & 0xffffff) << 2) as i32) << 6 >> 6) as u32).wrapping_add(8))
            as i32,
    };
    Ins::B { cond, target }
}
fn parse_thumb_b_0(value: u32) -> Ins {
    let cond = Cond::from(((value) >> 8) & 0xf);
    let target = BranchTarget {
        addr: (((((((value) & 0xff) << 1) as i32) << 23 >> 23) as u32).wrapping_add(4))
            as i32,
    };
    Ins::B { cond, target }
}
fn parse_thumb_b_1(value: u32) -> Ins {
    let cond = Cond::default();
    let target = BranchTarget {
        addr: (((((((value) & 0x7ff) << 1) as i32) << 20 >> 20) as u32).wrapping_add(4))
            as i32,
    };
    Ins::B { cond, target }
}
