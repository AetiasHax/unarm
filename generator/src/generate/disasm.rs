...
pub fn generate_disasm(isa: &Isa, isa_args: &IsaArgs, module: &str) -> Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        use crate::#module::disasm::{Ins, ParsedIns};
...
fn generate_field_accessor_body(
    ...
    if let Some(ops) = ops {
        ...
        let parse_value = generate_field_value_parse(arg, quote! { value });
        let return_value = if arg.signed {
            quote! { (#parse_value, #bits_literal) }
        } else {
            quote! { #parse_value }
        };
        quote! { {
        ...
        let parse_value = generate_field_value_parse(arg, quote! { (#base_value) });
        if arg.signed {
            quote! { { (#parse_value, #bits_literal) } }
        } else {
            quote! { { #parse_value } }
...
fn generate_field_value_parse(arg: &Arg, base_value: TokenStream) -> TokenStream {
    let arg_ident = Ident::new(&arg.variant_name(), Span::call_site());
    match (&arg.values, arg.boolean) {
